...
import com.example.cardinfinity.AddBusinessCardActivity;
...
public final class CaptureActivity extends Activity
	...
	public static final String EXTRA_BITMAP_BYTE_STREAM = "EXTRA_BITMAP_BYTE_STREAM";
	...
	boolean handleOcrDecode(OcrResult ocrResult) {
		...
		showBitmap = ocrResult.getOriBitmap();
		lastBitmap = ocrResult.getOriBitmap();
		// lastImage = (ImageView) findViewById(R.id.lastImage);
		// lastImage.setImageBitmap(lastBitmap);

//		if (lastBitmap == null) {
//			System.out.println("有往下跑");
...
//		}
		...
		if (ocrResult.getText() == null || ocrResult.getText().equals("")) {
			...
			return false;
		} else if (ocrResult.getText() != null) {
			if (lastBitmap == null) {
				System.out.println("有往下跑");
				bitmapImageView.setImageBitmap(BitmapFactory.decodeResource(getResources(), R.drawable.ic_launcher));
			} else {
			...
			bundle.putString("lastResult", ocrResult.getText().toString());
			bundle.putString("from", "CaptureActivity");
			Intent intent = new Intent();
			// 設定下一個Actitity
			intent.setClass(this, AddBusinessCardActivity.class);
			intent.putExtras(bundle);
			
			showBitmap.compress(Bitmap.CompressFormat.JPEG, 100, outImageByteArrayOutputStream);
	 ...
	 * http://www.androidengineer.com/2010/08/easy-method-for-formatting-android
	 * .html
	 * 
	 * <p>
	...
	private void setDefaultPreferences() {
		...
		// Continuous preview
		prefs.edit().putBoolean(PreferencesActivity.KEY_CONTINUOUS_PREVIEW, CaptureActivity.DEFAULT_TOGGLE_CONTINUOUS)
				.commit();

		// Recognition language
		prefs.edit().putString(PreferencesActivity.KEY_SOURCE_LANGUAGE_PREFERENCE,
				CaptureActivity.DEFAULT_SOURCE_LANGUAGE_CODE).commit();

		// Translation
		prefs.edit().putBoolean(PreferencesActivity.KEY_TOGGLE_TRANSLATION, CaptureActivity.DEFAULT_TOGGLE_TRANSLATION)
				.commit();

		// Translation target language
		prefs.edit().putString(PreferencesActivity.KEY_TARGET_LANGUAGE_PREFERENCE,
		...
		// OCR Engine
		prefs.edit().putString(PreferencesActivity.KEY_OCR_ENGINE_MODE, CaptureActivity.DEFAULT_OCR_ENGINE_MODE)
				.commit();
		...
		prefs.edit().putBoolean(PreferencesActivity.KEY_DISABLE_CONTINUOUS_FOCUS,
				CaptureActivity.DEFAULT_DISABLE_CONTINUOUS_FOCUS).commit();
		...
		prefs.edit().putString(PreferencesActivity.KEY_PAGE_SEGMENTATION_MODE,
				CaptureActivity.DEFAULT_PAGE_SEGMENTATION_MODE).commit();

		// Reversed camera image
		prefs.edit().putBoolean(PreferencesActivity.KEY_REVERSE_IMAGE, CaptureActivity.DEFAULT_TOGGLE_REVERSED_IMAGE)
				.commit();
